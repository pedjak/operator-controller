...
manifests: update-crds $(MANIFESTS) $(HELM) #EXHELP Generate OLMv1 manifests
	...
	@# Revert manifests/helm files where the only change is the controller-gen version annotation
	@for f in $$(git diff --name-only -- manifests/ helm/); do \
		git diff --quiet -I 'controller-gen.kubebuilder.io/version' -- "$$f" && git checkout -- "$$f" || true; \
	done
...
generate: $(CONTROLLER_GEN) #EXHELP Generate code containing DeepCopy, DeepCopyInto, DeepCopyObject, and ApplyConfiguration type implementations.
	...
	@for f in $$(git diff --name-only); do \
		git diff --quiet -I 'Code generated by controller-gen' -- "$$f" && git checkout -- "$$f" || true; \
	done
...
prometheus: PROMETHEUS_VERSION := v0.91.0
